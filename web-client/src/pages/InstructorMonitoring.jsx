...
export default function InstructorMonitoring({ accessToken, mode = 'progress' }) {
  ...
  useEffect(() => {
    ...
    async function loadCourses() {
      ...
      setError('');
      try {
        const body = await instructorApi.getOwnedCourses(accessToken);
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    async function loadReport() {
      ...
      try {
        const body = showResults
          ? await instructorApi.getCourseResultSummary(selectedCourseId, accessToken)
          : await instructorApi.getCourseProgress(selectedCourseId, accessToken);
    ...
    return () => { cancelled = true; };
  }, [accessToken, selectedCourseId, refreshKey, showResults]);

  const selectedCourse = courses.find(course => String(course.id) === selectedCourseId);
  const summary = report?.summary || {};
  const enrollmentItems = showResults ? [] : report?.items || [];
  const quizItems = showResults ? report?.quizzes || [] : [];
  const resultItems = showResults ? report?.results || [] : [];
  const viewTitle = showResults ? 'Quiz results' : 'Student progress';
  const viewDescription = showResults
    ? 'Review assessment participation and outcomes for each course you own.'
    : 'Follow active enrollments and saved course completion for each course you own.';

  if (coursesLoading) {
    return <section className="page-stack"><div className="card" role="status">Loading your courses...</div></section>;
  }

  return <section className="page-stack" data-instructor-view={mode}>
    <header className="page-intro">
      <p className="page-kicker">Instructor workspace</p>
      <h2 className="page-title">{viewTitle}</h2>
      <p className="page-description">{viewDescription}</p>
    </header>
    ...
    {courses.length === 0 ? <div className="card" role="status"><strong>No courses available</strong><p>Create a course before reviewing student activity.</p></div> : <>
      <div className="card revenue-toolbar">
        <div className="revenue-filter">
          ...
          <select id={`${mode}-course`} className="form-control" value={selectedCourseId} onChange={event => setSelectedCourseId(event.target.value)}>
      ...
      {reportLoading ? <div className="card" role="status">Loading {showResults ? 'quiz results' : 'student progress'}...</div> : report && (showResults ? <>
        <div className="metrics-grid">
          <StatCard eyebrow="Assessments" title="Published quizzes" value={summary.quizCount || 0} description="Available for enrolled students" tone="primary" />
          <StatCard eyebrow="Participation" title="Quiz attempts" value={summary.attemptCount || 0} description={`${summary.passedCount || 0} passing attempts`} />
          <StatCard eyebrow="Performance" title="Average score" value={`${summary.averagePercentage || 0}%`} description="Calculated from submitted attempts" tone="success" />
          <StatCard eyebrow="Completion" title="Passing attempts" value={summary.passedCount || 0} description={`${Math.max(0, Number(summary.attemptCount || 0) - Number(summary.passedCount || 0))} below pass mark`} />
        </div>

        <section className="card" aria-labelledby="quiz-summary-title">
          <div className="section-heading"><div><p className="section-label">Assessment overview</p><h2 id="quiz-summary-title">Quiz summary</h2></div><span className="section-heading__meta">{quizItems.length} quizzes</span></div>
        ...
        <section className="card" aria-labelledby="quiz-attempts-title">
          <div className="section-heading"><div><p className="section-label">Submitted work</p><h2 id="quiz-attempts-title">Student quiz attempts</h2></div><span className="section-heading__meta">{resultItems.length} attempts</span></div>
        ...
        <div className="metrics-grid">
          <StatCard eyebrow="Enrollment" title="Active students" value={summary.activeEnrollments || 0} description={`${summary.totalEnrollments || 0} total enrollments`} tone="primary" />
          <StatCard eyebrow="Course progress" title="Average progress" value={`${summary.averageProgress || 0}%`} description="Calculated from saved lesson completion" tone="success" />
          <StatCard eyebrow="Completion" title="Completed students" value={summary.completedEnrollments || 0} description={`${Math.max(0, Number(summary.activeEnrollments || 0) - Number(summary.completedEnrollments || 0))} still learning`} />
          <StatCard eyebrow="Course content" title="Lessons" value={selectedCourse?.lessonCount || 0} description="Included in progress calculation" />
        </div>
